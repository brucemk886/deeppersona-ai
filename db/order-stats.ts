...
export async function getOrderStats() {
  ...
  const rows = await getD1().prepare(`SELECT day, SUM(orders) AS orders FROM (
      SELECT date(paid_at, '+8 hours') AS day, COUNT(*) AS orders
      ...
      GROUP BY date(paid_at, '+8 hours')
      UNION ALL
      SELECT date(paid_at, '+8 hours') AS day, COUNT(*) AS orders
      FROM deep_orders WHERE livemode = 1 AND amount_cents > 0
      AND status IN ('paid', 'refunded') AND paid_at IS NOT NULL
      AND substr(id, 1, 8) != 'preview_'
      AND NOT EXISTS (SELECT 1 FROM quiz_reports r JOIN admin_test_sessions f ON f.session_id=r.session_id WHERE r.id=deep_orders.report_id)
      AND date(paid_at, '+8 hours') >= date('now', '+8 hours', '-13 days')
      AND date(paid_at, '+8 hours') <= date('now', '+8 hours')
      GROUP BY date(paid_at, '+8 hours')
    ) GROUP BY day`).all<{ day: string; orders: number }>();
