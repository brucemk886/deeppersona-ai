...
type RuntimeEnv = {
  CHECKOUT_RATE_LIMIT?: { limit: (input: { key: string }) => Promise<{ success: boolean }> };
  ADMIN_EMAILS?: string;
...
export async function submitQuiz(input: {
  report?: { id: string; snapshotJson: string; free: boolean };
  ...
  if (input.report) {
    writes.push(db.prepare(`INSERT INTO quiz_reports (id, session_id, profile_id, test_id, email, snapshot_json, free)
      VALUES (?, ?, ?, ?, ?, ?, ?)`)
      .bind(input.report.id, input.sessionId, input.profileId, input.testId, input.email, input.report.snapshotJson, input.report.free ? 1 : 0));
  }
