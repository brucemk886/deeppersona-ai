...
export function QuizApp({ initialTests, initialTestId, initialQuestions, initialReportId }: { initialTests: QuizTest[]; initialTestId?: string; initialQuestions: QuizQuestion[]; initialReportId?: string }) {
      ...
      <main className="test-detail-shell">
        ...
        <section className={`detail-stage ${textMode ? "detail-stage-text" : ""}`} style={{ "--test-accent": selectedTest.accent } as React.CSSProperties}>
          ...
          <div className="detail-story">
            ...
            {quizReady ? <div className="detail-reveal"><span>YOUR FREE RESULT INCLUDES</span><div><p>A primary style label and anxiety × avoidance map.</p><p>A first-look portrait of the contradiction underneath this style.</p><p>The optional full reading covers scenes, defenses, scripts, and a 7-day practice.</p></div></div> : null}
        ...
        <section className="email-gate">
          ...
          <form className="email-form" onSubmit={unlockResult}>
            ...
            <p>{normalizePresentationMode(selectedTest.presentationMode) === "text"
              ? "You have completed the questions. Enter your email to save your result and see your free summary. A longer four-part reading is optional."
    ...
    <main className="result-shell ap-result-shell">
      ...
      {result && selectedTest && deepResult ? (
        <article className="result-card result-card-expanded ap-paid">
          ...
          {showChoiceReview ? <section className="choice-review" aria-labelledby="choice-review-title">
            <header>
