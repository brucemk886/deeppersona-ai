...
import { isValidAffiliateProduct } from "@/lib/admin/validation";
...
export async function GET(request: Request) {
  ...
  try {
    return Response.json({ products: await listAffiliateProducts(true) });
  } catch (error) {
...
export async function PUT(request: Request) {
  ...
  if (!isValidAffiliateProduct(body)) {
    return Response.json({ error: "Invalid affiliate product" }, { status: 400 });
  }
