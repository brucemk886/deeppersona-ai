...
function formatTrafficDay(day: string, range: AdminStatsRange) {
  if (isHourlyAdminStatsRange(range)) return day.slice(11, 16);
...
export default function TrafficPanel({
  ...
  data?: TrafficData;
  onRangeChange: (value: AdminStatsRange) => void;
  ...
  return <>
    <div className="admin-page-heading">
      <div>
        ...
        <h1>流量分析</h1>
        <p>{rangeLabel} · 北京时间 · 排除已标记测试、沙盒及内部预览。更新于 {new Date(data.updatedAt).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai'})}</p>
      </div>
    ...
    <section className="metric-grid four">{[['访问次数',data.anonymous.pageviews],['开始测试',data.operations.started],['完成测试',data.operations.finished],['付款成功',data.operations.paid]].map(([label,value])=><div className="admin-card" key={label}><p>{label}</p><h2>{value}</h2><small>{rangeLabel}</small></div>)}</section>
