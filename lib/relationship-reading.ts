...
export function buildRelationshipReading(questions: QuizQuestion[], choices: Record<string, number>) {
  ...
  return { result, deepResult: { modules, lens: { title: 'A pattern, not a verdict', explanation: 'These scores describe how often your choices leaned toward reaching, stepping back, staying steady, or doing both. Your report also follows the wording you selected. A preference can have several explanations; it cannot establish childhood experiences, diagnose an attachment style, or reveal facts about another person.', reflectionPrompt: lead.ask } } };
}
