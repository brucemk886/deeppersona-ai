...
export function adminStatsRangeWindow(range: AdminStatsRange): {
  ...
  switch (range) {
    case "today":
      return {
        startExpr: "datetime('now', '+8 hours', 'start of day')",
        endExpr: "datetime('now', '+8 hours', '+1 day', 'start of day')",
        buckets: 24,
        hourly: true,
      };
    case "yesterday":
      return {
        startExpr: "datetime('now', '+8 hours', '-1 day', 'start of day')",
        endExpr: "datetime('now', '+8 hours', 'start of day')",
        buckets: 24,
        hourly: true,
      };
    case "7d":
      return {
        startExpr: "datetime('now', '+8 hours', '-6 days', 'start of day')",
        endExpr: null,
        buckets: 7,
        hourly: false,
      };
    case "30d":
      return {
        startExpr: "datetime('now', '+8 hours', '-29 days', 'start of day')",
        endExpr: null,
        buckets: 30,
        hourly: false,
      };
...
export function adminStatsTimePredicate(column: string, range: AdminStatsRange): string {
  ...
  column = `datetime(${column}, '+8 hours')`;
