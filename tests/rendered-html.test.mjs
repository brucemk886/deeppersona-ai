...
test("builds the complete DeepPersona AI experience", async () => {
  ...
  assert.match(admin, /label: "订单"/);
  assert.match(admin, /\/api\/admin\/payments/);
  assert.match(adminStatsRange, /今天/);
  assert.match(adminStatsRange, /昨天/);
  assert.match(adminStatsRange, /近7天/);
  assert.match(adminStatsRange, /近30天/);
  ...
  assert.match(store, /admin_test_sessions/);
  assert.match(store, /traffic_anonymous_pages/);
  assert.match(store, /payment_orders/);
  assert.match(adminStatsRange, /\+8 hours/);
